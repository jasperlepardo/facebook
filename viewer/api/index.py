...
function renderMedia(m) {
  ...
  if (m.files)       for (const f of m.files) { const name = f.uri.split('/').pop(); parts.push(`<div class="msg-flink">📎 <a href="${r2(f.uri)}" target="_blank">${esc(name)}</a></div>`); }
  ...
  if (m.call_duration != null) { const ds = m.call_duration ? `${Math.floor(m.call_duration/60)}m ${m.call_duration%60}s` : ''; parts.push(`<div class="msg-call">📞 ${m.missed ? 'Missed call' : `Call${ds?' · '+ds:''}`}</div>`); }
  if (m.reactions?.length) { const c = {}; for (const r of m.reactions) c[r.reaction]=(c[r.reaction]||0)+1; parts.push('<div class="msg-rea">'+Object.entries(c).map(([r,n])=>r+(n>1?' '+n:'')).join(' ')+'</div>'); }
  return parts.join('');
}
function renderMsgBody(m) { const med=renderMedia(m); const txt=m.is_unsent?'<div class="msg-unsent">Message removed</div>':(m.content?`<div class="msg-text">${esc(m.content)}</div>`:''); return txt+med; }
let lastDate=null,lastSender=null,lastTs=0;
function renderMessages(messages) {
  const blocks=[];
  for (const m of messages) {
    const d=fmtDate(m.timestamp_ms),newDate=d!==lastDate,grouped=!newDate&&m.sender_name===lastSender&&(m.timestamp_ms-lastTs)<5*60*1000;
    if(newDate)lastDate=d; lastSender=m.sender_name; lastTs=m.timestamp_ms;
    if(grouped&&blocks.length){blocks[blocks.length-1].msgs.push(m);}else{blocks.push({date:d,newDate,sender:m.sender_name,mine:m.sender_name===ME,msgs:[m]});}
  }
  return blocks.map(b=>{
    ...
    return `${sep}<div class="msg-group" data-id="${b.msgs[0]._id}" data-ts="${b.msgs[0].timestamp_ms}">
      ...
      <div class="msg-content"><div class="msg-header"><span class="msg-sender${b.mine?' me':''}">${esc(b.sender)}</span><span class="msg-ts">${ts}</span></div>${body}</div>
    </div>`;
  }).join('');
}
...
async function loadMessages(append=false,prepend=false) {
  ...
  const container=$('msgs'),count=data.messages.length;
  if(prepend){
    const prevH=$('chat').scrollHeight,prevTop=$('chat').scrollTop;
    const saved=[lastDate,lastSender,lastTs]; lastDate=null;lastSender=null;lastTs=0;
    const html=renderMessages(data.messages); [lastDate,lastSender,lastTs]=saved;
    container.insertAdjacentHTML('afterbegin',html);
    $('chat').scrollTop=prevTop+($('chat').scrollHeight-prevH);
  } else if(append){
    container.insertAdjacentHTML('beforeend',renderMessages(data.messages)); upperOffset+=count;
  } else {
    lastDate=null;lastSender=null;lastTs=0;
    container.innerHTML=renderMessages(data.messages); upperOffset=lowerOffset+count; $('chat').scrollTop=0;
  }
}
async function loadOlder(){if(lowerOffset===0)return;lowerOffset=Math.max(0,lowerOffset-LIMIT);await loadMessages(false,true);}
async function loadNewer(){await loadMessages(true);}
async function jumpToEnd(){lowerOffset=Math.max(0,total-LIMIT);upperOffset=lowerOffset;searchQ='';$('search').value='';lastDate=null;await loadMessages(false);$('chat').scrollTop=$('chat').scrollHeight;}
...
function lb(src,type,caption=''){const inner=$('lbinner');if(type==='video'){inner.innerHTML=`<video src="${src}" controls autoplay style="max-width:92vw;max-height:88vh;border-radius:4px"></video>`;}else{inner.innerHTML=`<img src="${src}" alt="">`;}$('lbcap').textContent=caption;$('lb').classList.add('on');}
function closeLb(){$('lb').classList.remove('on');$('lbinner').innerHTML='';}
$('lb').addEventListener('click',e=>{if(e.target===$('lb'))closeLb();});
document.addEventListener('keydown',e=>{if(e.key==='Escape'){closeLb();closeNoteModal();closeCtxMenu();}});
document.querySelectorAll('.tab').forEach(t=>t.addEventListener('click',()=>{
  document.querySelectorAll('.tab').forEach(x=>x.classList.remove('on'));t.classList.add('on');currentTab=t.dataset.tab;
  $('chat').style.display='none';$('gallery').style.display='none';$('fview').style.display='none';
...
function fmtIso(iso){if(!iso)return'';const dateStr=iso.split('T')[0];const d=new Date(dateStr+'T00:00:00');const datePart=d.toLocaleDateString([],{month:'short',day:'numeric',year:'numeric'});if(!hasTime(iso))return datePart;const[h,m]=iso.split('T')[1].split(':').map(Number);const ampm=h>=12?'PM':'AM';const h12=h%12||12;return datePart+' · '+h12+':'+String(m).padStart(2,'0')+' '+ampm;}
function fmtDateRange(start,end){if(start===end)return fmtIso(start);const sd=start.split('T')[0],ed=end.split('T')[0];if(sd===ed){const d=new Date(sd+'T00:00:00').toLocaleDateString([],{month:'short',day:'numeric',year:'numeric'});if(!hasTime(start)&&!hasTime(end))return d;const fmtT=iso=>{const[h,m]=iso.split('T')[1].split(':').map(Number);const ap=h>=12?'PM':'AM';return(h%12||12)+':'+String(m).padStart(2,'0')+' '+ap;};const t1=hasTime(start)?fmtT(start):'';const t2=hasTime(end)?fmtT(end):'';return d+(t1||t2?' · '+[t1,t2].filter(Boolean).join(' – '):'');}const s=new Date(sd+'T00:00:00'),e=new Date(ed+'T00:00:00');if(s.getMonth()===e.getMonth()&&s.getFullYear()===e.getFullYear()){return s.toLocaleDateString([],{month:'short',day:'numeric'})+' – '+e.toLocaleDateString([],{day:'numeric',year:'numeric'});}return fmtIso(start)+' – '+fmtIso(end);}
function renderNotes(notes){const body=$('notes-body');if(!notes.length){body.innerHTML='<p style="padding:16px;color:#65676b;font-size:13px">No notes match.</p>';return;}body.innerHTML=notes.map((n,i)=>{const accentColor=TAG_COLORS[n.tags[0]]||'#e4e6ea';const tagText=n.tags.map(t=>TAG_LABELS[t]||t).join(' · ');const bodyHtml=esc(n.body).replace(/\n/g,'<br>');const dateRange=fmtDateRange(n.start,n.end);return`<div class="note-card" data-idx="${i}" data-id="${n.id}" data-start="${n.start}" data-end="${n.end}" style="border-left-color:${accentColor}"><div class="note-date"><span>${dateRange}</span><div style="display:flex;gap:6px;align-items:center"><button class="note-edit-btn" data-id="${n.id}">✏</button><button class="jump-btn" data-start="${n.start}" data-end="${n.end}">→ Chat</button></div></div><div class="note-tags">${tagText}</div><div class="note-title">${esc(n.title)}</div><div class="note-body">${bodyHtml}</div></div>`;}).join('');}
document.addEventListener('click',async function(e){
  const jumpBtn=e.target.closest('.jump-btn');if(jumpBtn){e.stopPropagation();await jumpToDate(jumpBtn.dataset.start);return;}
...
let _editingId=null;
function openNoteModal(note){_editingId=note?note.id:null;$('note-dlg-h').textContent=note?'Edit Note':'New Note';$('nf-start').value=note?(note.start||''):'';$('nf-end').value=note?(note.end||''):'';$('nf-title').value=note?(note.title||''):'';$('nf-body').value=note?(note.body||''):'';const activeTags=note?(note.tags||[]):[];$('nf-tags').innerHTML=ALL_TAGS.map(t=>`<span class="tag-chip${activeTags.includes(t)?' on':''}" data-tag="${t}">${t}</span>`).join('');$('nf-tags').querySelectorAll('.tag-chip').forEach(chip=>{chip.addEventListener('click',()=>chip.classList.toggle('on'));});$('nf-delete').style.visibility=note?'visible':'hidden';$('note-modal').classList.add('on');$('nf-title').focus();}
function closeNoteModal(){$('note-modal').classList.remove('on');_editingId=null;}
async function saveNote(){const btn=$('nf-save');btn.disabled=true;const payload={start:$('nf-start').value.trim(),end:$('nf-end').value.trim()||null,title:$('nf-title').value.trim(),body:$('nf-body').value.trim(),tags:[...$('nf-tags').querySelectorAll('.tag-chip.on')].map(c=>c.dataset.tag)};try{if(_editingId){await fetch(`/api/notes/${_editingId}`,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});}else{await fetch('/api/notes',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});}closeNoteModal();allNotes=await api('/api/notes');filterNotes($('notes-filter').value);}catch(err){alert('Save failed: '+err);}finally{btn.disabled=false;}}
async function deleteNote(){if(!_editingId||!confirm('Delete this note?'))return;try{await fetch(`/api/notes/${_editingId}`,{method:'DELETE'});closeNoteModal();allNotes=await api('/api/notes');filterNotes($('notes-filter').value);}catch(err){alert('Delete failed: '+err);}}
let _ctxNote=null;
function closeCtxMenu(){$('ctx-menu').classList.remove('on');_ctxNote=null;}
document.addEventListener('contextmenu',e=>{const card=e.target.closest('.note-card');if(!card)return;e.preventDefault();_ctxNote=allNotes.find(n=>String(n.id)===card.dataset.id);if(!_ctxNote)return;const menu=$('ctx-menu');menu.style.left=Math.min(e.clientX,window.innerWidth-menu.offsetWidth-8)+'px';menu.style.top=Math.min(e.clientY,window.innerHeight-menu.offsetHeight-8)+'px';menu.classList.add('on');});
$('ctx-edit').addEventListener('click',()=>{const note=_ctxNote;closeCtxMenu();if(note)openNoteModal(note);});
document.addEventListener('click',e=>{if(!e.target.closest('#ctx-menu'))closeCtxMenu();},true);
(function(){const resizer=$('resizer'),pane=$('notes-pane');let startX,startW;resizer.addEventListener('mousedown',e=>{startX=e.clientX;startW=pane.offsetWidth;resizer.classList.add('dragging');document.body.style.userSelect='none';document.body.style.cursor='col-resize';});document.addEventListener('mousemove',e=>{if(!resizer.classList.contains('dragging'))return;const delta=startX-e.clientX;const newW=Math.max(220,Math.min(window.innerWidth*0.6,startW+delta));pane.style.width=newW+'px';});document.addEventListener('mouseup',()=>{resizer.classList.remove('dragging');document.body.style.userSelect='';document.body.style.cursor='';});})();
let loading=false;
$('chat').addEventListener('scroll',()=>{if(loading||searchQ||currentTab!=='chat')return;const el=$('chat');if(el.scrollTop<80&&lowerOffset>0){loading=true;loadOlder().finally(()=>{loading=false;});}if(el.scrollTop+el.clientHeight>=el.scrollHeight-80){const btn=$('loadbtn');if(btn.style.display==='none')return;loading=true;loadNewer().finally(()=>{loading=false;});}});
async function initChat(){lowerOffset=0;upperOffset=0;await loadMessages(false);$('chat').style.visibility='';}
initChat();
